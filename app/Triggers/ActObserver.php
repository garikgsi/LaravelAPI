...
class ActObserver
{
    ...
    public function saving(Act $a)
    {
        ...
        if ($a->getOriginal("is_active") == 1) {
            ...
            if ($a->isDirty('sklad_id')) {
                abort(421, '#AO.Чтобы изменить склад необходимо сначала распровести документ');
                return false;
            }
        }
        ...
        if ($a->isDirty('is_active') && $a->is_active == 1) {
            ...
            if (!$is_keeper && !$is_admin) {
                abort(421, '#AO.Проводить накладные может только кладовщик или администратор');
                return false;
            } else {
                ...
                if (count($ostatok_err)) {
                    $err = "#AO.Недостаточно: " . implode(", ", $ostatok_err);
                    abort(421, $err);
                    return false;
                }
            }
        ...
        if ($a->isDirty('is_active') && $a->is_active == 0) {
            // проводить только кладовщик склада отправления
            if (!$is_keeper && !$is_admin) {
                abort(421, '#AO.Распроводить накладные может только кладовщик или администратор');
                return false;
            }
        }
    }
    ...
    public function saved(Act $a)
    {
        ...
        if ($a->isDirty('is_active') || $a->isDirty('doc_date') || $a->isDirty('sklad_id')) {
